...
import { createOrUpdateFile, createBranch, createPullRequest } from "@/lib/github";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { slug, frontmatter, content, createPR = true } = await request.json();
    ...
    if (createPR) {
      ...
      const prNumber = await createPullRequest(
        branchName,
        `Add new blog post: ${frontmatter.title || slug}`,
        `This PR adds a new blog post "${frontmatter.title || slug}".\n\nCreated via ETSA Admin interface by ${session.user.name} (${session.user.email}).`
      );
